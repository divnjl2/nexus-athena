...
## Athena adaptation — autonomous mode

When driven by Hermes with no human at the keyboard, the approval in step 8 is
satisfied by the `planner_question` MCP verb: Hermes answers each design fork from
project policy/context, or decisions are pre-seeded in the ticket. NEVER hang waiting
for "magic words" — if a fork has no answer source, record it as an open decision in
`questions.md` and let Hermes resolve it. (Closes RPI failure mode #1.)
